...
#include "BinarySearchTree.hpp"  // Include the implementation for this unit.
...
int main() {  // CLI entry point for the demo.
    std::vector<int> values = {8, 3, 10, 1, 6, 14, 4, 7, 13};  // Classic BST insert order used in many textbooks.
    bstunit::BinarySearchTree tree = bstunit::BinarySearchTree::fromValues(values);  // Build BST by inserting in order.
    bstunit::TreeSummary s = tree.summarize();  // Summarize properties and inorder traversal.

    std::cout << "=== Binary Search Tree Demo (C++) ===\n";  // Print header.
    std::cout << "insert order: " << formatVector(values) << "\n";  // Print insertion order.
    std::cout << "size=" << s.size << ", height=" << s.height << ", min=" << formatOptionalInt(s.minValue) << ", max=" << formatOptionalInt(s.maxValue) << "\n";  // Print key properties.
    ...
    tree.remove(3);  // Delete a two-child node (3) using successor replacement.
    std::cout << "after deletes (7,14,3) inorder=" << formatVector(tree.inorder()) << "\n";  // Show inorder after deletions.
