...
#include "Queue.hpp"  // Import the queue implementation and helpers.
...
static void printEnqueueGrowthTable(const std::vector<int>& ms) {  // Print enqueue-growth summary table.
    ...
    for (int m : ms) {  // Render one row per m.
        queueunit::EnqueueSummary s = queueunit::simulateEnqueues(m);  // Simulate and summarize.
...
static void printDequeueCostTable(const std::vector<int>& ns) {  // Print dequeue shift-cost table.
    std::cout << std::setw(6) << "n" << " | " << std::setw(6) << "moved" << "\n";  // Header.
    std::cout << "-----------------\n";  // Separator line.
    for (int n : ns) {  // Render one row per n.
        int moved = (n == 0) ? 0 : queueunit::simulateDequeueCostAtSize(n).moved;  // Skip invalid dequeue on empty queue.
        std::cout << std::setw(6) << n << " | " << std::setw(6) << moved << "\n";  // Row.
    }  // Close loop.
}  // End printDequeueCostTable().

static void printFifoDemo() {  // Print a small FIFO example with peek/dequeue.
    queueunit::ArrayQueue q;  // Create an empty queue.
...
int main(int argc, char** argv) {  // Entry point for demo CLI.
    ...
    std::cout << "\n";  // Blank line.
    std::cout << "=== Dequeue Shift Cost (dequeue once from size n) ===\n";  // Print section title.
