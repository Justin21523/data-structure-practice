// 02 佇列示範程式（C）/ Queue demo program (C).  // Bilingual file header.

#include "queue.h"  // Use the queue API for this unit.
...
static enqueue_summary simulate_enqueues(int m) {  // Simulate m enqueues and summarize growth/copy costs.
    ...
    array_queue q;  // Allocate queue struct on the stack frame.
    if (!array_queue_init(&q)) {  // Initialize backing buffer.
    ...
    for (int i = 0; i < m; i++) {  // Perform m enqueues.
        ...
        if (!array_queue_enqueue(&q, i, &cost)) {  // Enqueue deterministic value.
            fprintf(stderr, "allocation failed\n");  // Print error.
            exit(1);  // Exit demo with failure.
        }  // Close enqueue check.
    ...
    array_queue_destroy(&q);  // Free queue resources.
...
static int simulate_dequeue_moved_at_size(int n) {  // Build size n queue then dequeue once and return moved.
    ...
    array_queue q;  // Allocate queue struct.
    if (!array_queue_init(&q)) {  // Initialize queue.
        fprintf(stderr, "allocation failed\n");  // Print error.
        exit(1);  // Exit failure.
    }  // Close init check.
    queue_cost c;  // Reusable cost record for enqueues.
    for (int i = 0; i < n; i++) {  // Enqueue 0..n-1.
        if (!array_queue_enqueue(&q, i, &c)) {  // Enqueue one value.
            fprintf(stderr, "allocation failed\n");  // Print error.
            exit(1);  // Exit failure.
        }  // Close enqueue check.
    }  // Close loop.
    queue_dequeue_result r;  // Dequeue result record.
    if (!array_queue_dequeue(&q, &r)) {  // Dequeue once.
        fprintf(stderr, "dequeue failed\n");  // Print error.
        exit(1);  // Exit failure.
    }  // Close dequeue check.
    int moved = r.cost.moved;  // Capture moved count.
    array_queue_destroy(&q);  // Free queue resources.
...
static void print_dequeue_cost_table(const int* ns, int ns_len) {  // Print dequeue shift-cost table.
    printf("%6s | %6s\n", "n", "moved");  // Print header.
    printf("-----------------\n");  // Print separator.
    for (int i = 0; i < ns_len; i++) {  // Render one row per n.
        int moved = simulate_dequeue_moved_at_size(ns[i]);  // moved should equal max(0,n-1).
...
static void print_fifo_demo(void) {  // Print a small FIFO example with peek/dequeue.
    ...
    queue_cost c;  // Reusable cost record for enqueues.
    array_queue_enqueue(&q, 3, &c);  // Enqueue 3.
    array_queue_enqueue(&q, 7, &c);  // Enqueue 7.
    array_queue_enqueue(&q, 1, &c);  // Enqueue 1.

    int front = 0;  // Store peeked front value.
    array_queue_peek(&q, &front);  // Peek current front.
    ...
    array_queue_dequeue(&q, &d1);  // Dequeue first (should be 3).
    array_queue_dequeue(&q, &d2);  // Dequeue second (should be 7).
    array_queue_dequeue(&q, &d3);  // Dequeue third (should be 1).
...
int main(int argc, char** argv) {  // Entry point for demo CLI.
    ...
    int ns[] = {0, 1, 2, 4, 8, 16};  // Fixed n list for dequeue costs.
    ...
    printf("\n");  // Blank line.
    printf("=== Dequeue Shift Cost (dequeue once from size n) ===\n");  // Print section title.
