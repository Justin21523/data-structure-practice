// 01 堆疊示範（Java）/ Stack demo (Java).  // Bilingual file header.
...
public final class StackDemo {  // Provide an array-backed stack implementation with explicit resize-copy costs.  
    private StackDemo() {  // Prevent instantiation because this class is a container for static logic.  
        // No instances.  // Clarify intent.  
    }  // Close constructor.  

    public static final class OperationCost {  // Record per-operation resize copy counts.  
    ...
    public static final class PopResult {  // Return popped value plus its operation cost.  
        ...
        public final OperationCost cost;  // Resize-copy cost for this pop (always 0 in this unit).  
    ...
    public static final class ArrayStack {  // An array-backed stack with doubling growth (teaching-oriented).  
        ...
        public ArrayStack() {  // Initialize an empty stack with capacity 1.  
            this.size = 0;  // Empty stack has size 0.  
        ...
        public int[] toArray() {  // Return a copy of the used portion (bottom -> top).  
        ...
        public OperationCost push(int value) {  // Push to top (amortized O(1)).  
            ...
            this.data[this.size] = value;  // Write new value at top slot.  
            this.size += 1;  // Increase size.  
            return new OperationCost(copied);  // Return resize-copy cost record.  
        }  // End push().  

        public int peek() {  // Return top value without removing it (O(1)).  
            if (this.size == 0) {  // Reject peeking an empty stack.  
                throw new IndexOutOfBoundsException("peek from empty stack");  // Signal invalid operation.  
            }  // Close validation.  
            return this.data[this.size - 1];  // Return top slot.  
        }  // End peek().  

        public PopResult pop() {  // Pop top value (O(1) in this unit; no shrinking).  
            if (this.size == 0) {  // Reject popping an empty stack.  
                throw new IndexOutOfBoundsException("pop from empty stack");  // Signal invalid operation.  
            }  // Close validation.  
            this.size -= 1;  // Decrease size first so top index becomes size.  
            int v = this.data[this.size];  // Read removed slot.  
            return new PopResult(v, new OperationCost(0));  // Pop does not resize/copy in this unit.  
        }  // End pop().  
    }  // End ArrayStack.  

    public static PushSummary simulatePushes(int m) {  // Simulate m pushes and summarize growth/copy costs.  
        if (m < 0) {  // Reject invalid counts.  
            throw new IllegalArgumentException("m must be >= 0");  // Signal invalid input.  
        }  // Close validation.  
        ArrayStack s = new ArrayStack();  // Fresh stack for deterministic results.  
        ...
        for (int i = 0; i < m; i++) {  // Perform m pushes.  
            OperationCost cost = s.push(i);  // Push deterministic value.  
            totalActualCost += 1L + cost.copied;  // Add write + copy cost.  
            maxCopied = Math.max(maxCopied, cost.copied);  // Update max copied.  
        }  // Close loop.  
        return new PushSummary(m, s.size(), s.capacity(), s.totalCopies(), totalActualCost, maxCopied);  // Return summary.  
    }  // End simulatePushes.  
    ...
    private static String formatPushSummaryTable(List<Integer> ms) {  // Format growth summaries for multiple m values.  
        ...
        for (int m : ms) {  // Render one row per m.  
            PushSummary s = simulatePushes(m);  // Simulate and summarize.  
    ...
    private static String formatLifoDemo() {  // Show a small LIFO example with peek/pop.  
        ArrayStack s = new ArrayStack();  // Create an empty stack.  
        s.push(3);  // Push 3.  
        s.push(7);  // Push 7.  
        ...
        return "push [3,7,1] => peek=" + top + ", pops=" + Arrays.toString(new int[] {p1, p2, p3});  // Return formatted demo line.  
    }  // End formatLifoDemo.  
    ...
    public static void main(String[] args) {  // CLI entry point for the demo.  
        ...
        System.out.println("=== Push Growth (m pushes) ===");  // Print section title.  
        System.out.print(formatPushSummaryTable(ms));  // Print table.  
        System.out.println();  // Blank line.  
        System.out.println("=== LIFO demo ===");  // Print section title.  
        System.out.println(formatLifoDemo());  // Print LIFO demo line.  
    }  // End main.  
}  // End StackDemo.  
