// 03 環狀佇列示範（Java）/ Circular queue demo (Java).  // Bilingual file header.
...
public final class CircularQueueDemo {  // Provide a circular array-backed queue implementation with explicit copy/shift costs.  
    private CircularQueueDemo() {  // Prevent instantiation because this class is a container for static logic.  
    ...
    public static final class OperationCost {  // Record per-operation copy/shift counts.  
        ...
        public final int moved;  // How many elements were shifted due to dequeue (should be 0 here).  
    ...
    public static final class CircularQueue {  // A circular array-backed queue (dequeue does not shift; teaching-oriented).  
        ...
        public int[] toArray() {  // Return a copy of the used portion (front -> rear).  
            ...
            for (int i = 0; i < this.size; i++) {  // Copy elements in queue order.  
        ...
        private int resize(int newCapacity) {  // Resize buffer and return number of copied elements.  
            ...
            this.head = 0;  // Reset head so queue starts at index 0 after resize.  
        ...
        public DequeueResult dequeue() {  // Remove and return front value (O(1); no shifting).  
            ...
            if (this.size == 0) {  // Normalize head for deterministic state when queue becomes empty.  
                this.head = 0;  // Reset head (optional but stable).  
            }  // Close normalize branch.  
            return new DequeueResult(removed, new OperationCost(0, 0));  // moved stays 0 (no shift).  
        }  // End dequeue().  
    }  // End CircularQueue.  
    ...
    public static EnqueueSummary simulateEnqueues(int m) {  // Simulate m enqueues and summarize growth/copy costs.  
        ...
        CircularQueue q = new CircularQueue();  // Fresh queue for deterministic results.  
    ...
    public static CircularQueue buildOrderedQueue(int n) {  // Build a queue containing [0, 1, ..., n-1].  
        if (n < 0) {  // Reject invalid sizes.  
            throw new IllegalArgumentException("n must be >= 0");  // Signal invalid input.  
        }  // Close validation.  
        CircularQueue q = new CircularQueue();  // Create empty queue.  
    ...
    private static String formatDequeueCostTable(int[] ns) {  // Show dequeue moved costs at multiple sizes (should be 0).  
    ...
    private static String formatFifoDemo() {  // Show a small FIFO example with peek/dequeue.  
        CircularQueue q = new CircularQueue();  // Create an empty queue.  
    ...
    public static void main(String[] args) {  // CLI entry point for the demo.  
        ...
        System.out.println("=== Dequeue Shift Cost (should be 0) ===");  // Print section title.  
