"""環狀佇列（Circular Queue）示範 / Circular queue demo.  # Bilingual module header.

本單元用「環狀索引（circular indexing）」實作佇列（queue），避免 dequeue 左移：  # Traditional Chinese overview.
- 只維護 head（front index）與 size  # Explain representation.
- tailIndex = (head + size) % capacity  # Explain how to find enqueue position.
- dequeue 只做 head = (head + 1) % capacity，不搬移元素  # Explain why moved=0.
...
- moved: number of elements shifted during dequeue (should always be 0 here)  # Shift cost (eliminated).
...
class OperationCost:  # Record per-operation copy/shift counts.  
    ...
    moved: int  # How many elements were shifted due to dequeue (0 in circular queue).  
...
class CircularQueue:  # A circular array-backed queue (dequeue does not shift; teaching-oriented).  
    ...
    def to_list(self) -> List[int]:  # Return a copy of the used portion (front -> rear).  
        ...
        for i in range(self._size):  # Copy exactly size elements in queue order.  
    ...
    def _resize(self, new_capacity: int) -> int:  # Resize buffer and return number of copied elements.  
        ...
        for i in range(self._size):  # Copy elements in queue order.  
        ...
        self._head = 0  # Reset head so queue starts at index 0 after resize.  
    ...
    def enqueue(self, value: int) -> OperationCost:  # Add to rear (amortized O(1)).  
        ...
        return OperationCost(copied=copied, moved=0)  # Enqueue does not shift elements.  
    ...
    def dequeue(self) -> DequeueResult:  # Remove and return front value (O(1); no shifting).  
        ...
        if self._size == 0:  # Normalize head for deterministic state when queue becomes empty.  
            self._head = 0  # Reset head so next enqueue starts from index 0 (optional but stable).  
        return DequeueResult(value=removed, cost=OperationCost(copied=0, moved=0))  # moved stays 0 (no shift).  
...
def simulate_enqueues(m: int) -> EnqueueSummary:  # Simulate m enqueues and summarize growth/copy costs.  
    ...
    q = CircularQueue()  # Use a fresh queue so results depend only on m.  
...
def build_ordered_queue(n: int) -> CircularQueue:  # Build a queue containing [0, 1, ..., n-1].  
...
def format_dequeue_cost_table(ns: Sequence[int]) -> str:  # Show dequeue moved costs at multiple sizes (should be 0).  
...
def main(argv: List[str]) -> int:  # CLI entry point for the demo.  
    ...
    ns = [0, 1, 2, 4, 8, 16]  # Use a small fixed set for dequeue moved costs.  
    print("=== Enqueue Growth (m enqueues) ===")  # Print section title.  
    print(format_enqueue_summary_table(ms))  # Print enqueue growth table.  
    print()  # Blank line.  
    print("=== Dequeue Shift Cost (should be 0) ===")  # Print section title.  
