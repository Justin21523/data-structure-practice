# 實作說明（Java）— Red-Black Tree（LLRB 版本）

## 目標

本單元用 Java 手刻 `RedBlackTree`，採用紅黑樹的等價變體 **Left-Leaning Red-Black Tree（LLRB）**：

- LLRB 仍滿足紅黑樹性質，但額外要求「紅連結向左」，因此插入/刪除可以用一致的遞迴流程完成
- 本 repo **不允許重複 key**：重複插入回傳 `false`
- `height` 以「邊數」定義：空樹 `-1`、葉 `0`（此單元高度用遞迴計算）

## 檔案

- `RedBlackTreeDemo.java`：資料結構 + demo 輸出
- `RedBlackTreeDemoTest.java`：不依賴 JUnit 的自製測試（離線可跑）
...
刪除用 `moveRedLeft/moveRedRight`，在往下走之前先確保要走的那一側不是 2-node，避免走到底後無法修復。
