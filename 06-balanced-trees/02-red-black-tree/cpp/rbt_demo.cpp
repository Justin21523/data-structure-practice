...
#include "RedBlackTree.hpp"  // Include the implementation for this unit.
...
int main() {  // CLI entry point for the demo.
    ...
    rbtunit::RedBlackTree tree = rbtunit::RedBlackTree::fromValues(values);  // Build tree by inserting in order.
    rbtunit::TreeSummary s = tree.summarize();  // Summarize properties and inorder traversal.

    std::cout << "=== Red-Black Tree Demo (C++, LLRB) ===\n";  // Print header.
    ...
    tree.remove(14);  // Delete a one-child node in this data set.
